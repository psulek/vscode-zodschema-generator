...
export class ShowPreviewCommand {
    ...
    private async updatePreviewWindow(updateMode: PreviewUpdateMode, textEditor?: vscode.TextEditor): Promise<void> {
        try {
            ...
            //const title = `Zod types preview - ${path.parse(originFilename).base}`;
            ...
            const content = result.getZodSchemasFile('');


            vscode.workspace.openTextDocument({language: 'typescript', content }).then(doc => {
